...
$load_balance = new LoadBalanceTest([
    'max_polling' => 10000000,
    'default_target_id' => 1,
]);
$load_balance->setServiceName('stock_service');

$stat_key = 'service:test:stat';
$redis = $load_balance->getRedis();

for ($i = 0; $i < 1000000; ++$i) {
    $target = $load_balance->getTarget();
    $redis->hIncrBy($stat_key, $target, 1);
}
